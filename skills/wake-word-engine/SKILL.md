...
description: Always-listening wake word engine using openWakeWord/Porcupine with stop interrupt and follow-up mode.
tier: A
status: planned
allowed-tools:
  - microphone
...
Always-listening wake word engine using openWakeWord/Porcupine with stop interrupt and follow-up mode.

## Current Jarvis integration

Status: `planned`. Tier: `A`.

- Callable from the cockpit through `POST /api/skills/wake-word-engine/run`.
- Callable from CLI with `npm run jarvis -- run wake-word-engine -- <args>`.
...
```bash
npm run jarvis -- run wake-word-engine -- demo
```
