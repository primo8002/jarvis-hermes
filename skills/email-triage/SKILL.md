---
name: email-triage
description: Gmail/Outlook/iCloud triage/digest/reply drafting through configured MCP/Hermes email tools.
tier: B
status: bridge
allowed-tools:
  - mcp
...
Gmail/Outlook/iCloud triage/digest/reply drafting through configured MCP/Hermes email tools.

## Current Jarvis integration

Status: `bridge`. Tier: `B`.

- Callable from the cockpit through `POST /api/skills/email-triage/run`.
- Callable from CLI with `npm run jarvis -- run email-triage -- <args>`.
...
```bash
npm run jarvis -- run email-triage -- demo
```
