...
description: Visible Brave/Chrome automation built on the Cowork browser endpoint; roadmap includes forms, tabs, and vault login.
tier: C
status: native
allowed-tools:
  - browser
...
Visible Brave/Chrome automation built on the Cowork browser endpoint; roadmap includes forms, tabs, and vault login.

## Current Jarvis integration

Status: `native`. Tier: `C`.

- Callable from the cockpit through `POST /api/skills/browser-operator/run`.
- Callable from CLI with `npm run jarvis -- run browser-operator -- <args>`.
...
```bash
npm run jarvis -- run browser-operator -- demo
```
