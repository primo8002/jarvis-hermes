---
name: calendar-skill
description: Calendar read/write/scheduling through Google Workspace/MCP when credentials are enabled.
tier: B
status: bridge
allowed-tools:
  - mcp
...
Calendar read/write/scheduling through Google Workspace/MCP when credentials are enabled.

## Current Jarvis integration

Status: `bridge`. Tier: `B`.

- Callable from the cockpit through `POST /api/skills/calendar-skill/run`.
- Callable from CLI with `npm run jarvis -- run calendar-skill -- <args>`.
...
```bash
npm run jarvis -- run calendar-skill -- demo
```
